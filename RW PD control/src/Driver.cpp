...
void Driver::begin() 
{
    ...
    drv_write(0x00, 0x2122);
    Serial.print("0x00 expect 0x2122, got 0x"); Serial.println(drv_read(0x00), HEX);
    ...
    Serial.print("0x0B expect 0x0000, got 0x"); Serial.println(drv_read(0x0B), HEX);
...
/** @brief A function which commands a direct PWM to the MOSFETS
 * 
 *  @details This function commands the PD controller input to the MOSFETS.
 * 
 *  @param SPEED_CMD The desired rpm speed of the motor.
 */
 ...
 * 
 *  @param threshold_edges The new edge count threshold for the PCNT event. 
 *  This is the number of edges on FGOUT after which the ISR will trigger and timestamp will be taken.
 */
