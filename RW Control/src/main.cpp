 ...
 *  there is a value in the edge_time queue, so it runs at the period of the motor spinning. This 
 *  task runs at a fixed rate of 100 Hz to avoid aperiodicity in the control architecture.
*/
void task_readActual(void* parameters) 
{
    ...
    while (true) 
    {
        ...
        // if(rpm_filt - rpm_filt_prev > -50.0f)
        // {
...
void task_gainScheduling(void* parameters)
{
    float speed_real = 0.0f;
 
    // Track current gain state (band)
    enum GainBand { BAND_LOW, BAND_MID, BAND_HIGH };
    GainBand current_band = BAND_LOW;
 
    // Store last written values so we only write on change
    uint16_t FILK2_last = 0xFFFF;
    uint16_t COMPK1_last = 0xFFFF;
    uint16_t COMPK2_last = 0xFFFF;
 
    while (true)
    {
        speed_real = speed_actual.get();
 
        GainBand new_band = current_band;
 
        // --- Hysteresis thresholds ---
        // Prevent rapid toggling near boundaries
        if (current_band == BAND_LOW)
        {
            if (speed_real > 2050.0f) new_band = BAND_MID;
        }
        else if (current_band == BAND_MID)
        {
            if (speed_real < 1950.0f) new_band = BAND_LOW;
            else if (speed_real > 2300.0f) new_band = BAND_HIGH;
        }
        else if (current_band == BAND_HIGH)
        {
            if (speed_real < 2200.0f) new_band = BAND_MID;
        }
        ...
        if (new_band != current_band)
        {
            ...
            if (current_band == BAND_LOW)
            {
                FILK2 = (uint16_t)(0.4714f * speed_real + 300.0f); // RW3 (white not noisy)
                // FILK2 = (uint16_t)(0.4714f * speed_real + 180.0f); // RW0 (gray)
                // FILK2 = (uint16_t)(0.4714f * speed_real + 120.0f); // RW2 (caleb)
                // FILK2 = (uint16_t)(0.4714f * speed_real + 400.0f); // RW1 (white noisy)
                COMPK1 = 1000;
                COMPK2 = 1000;
                // COMPK1 = 750; // for RW1 
                // COMPK2 = 1150; // for RW1
            ...
            if (FILK2 != FILK2_last)
            {
                Peripheral.drv_write(0x07, FILK2);
                FILK2_last = FILK2;
            }
 
            if (COMPK1 != COMPK1_last)
            {
                Peripheral.drv_write(0x08, COMPK1);
                COMPK1_last = COMPK1;
            }
 
            if (COMPK2 != COMPK2_last)
            {
                Peripheral.drv_write(0x09, COMPK2);
                COMPK2_last = COMPK2;
            }
        }
 
        // Light polling delay (does NOT constantly rewrite registers)
...
void task_speedControl(void* parameters)
{
    ...
    {        
        ...
        if (speed_state == 0) 
        {
            ...
            if (speed_command > speed_real && fabsf(speed_command) > cmd_deadband) 
            {
                if ((sign(speed_command) == sign(speed_real) || sign(speed_real) == 0) && (speed_command - speed_real > deadband)) 
                {
                    ...
                    if (direction == LOW) 
                    {
                        if(speed_command - speed_real > 100.0f)
                        {
                            // Command full duty
                            // Peripheral.cmd_speed_PWM(fabsf(speed_command)); 
                            Peripheral.drv_write(0x00, 0x2122);
                            Peripheral.cmd_speed_SPI(-1);

                            // accel
                            speed_state = 1;
                            // Serial.println("0 -> 1 + or zero > ++");
                        }
                        else
                        {
                            // Command control loop 
                            // Peripheral.cmd_speed_PWM(fabsf(speed_command)); 
                            Peripheral.drv_write(0x00, 0x2102);
                            Peripheral.cmd_speed_PWM(fabsf(speed_command));

                            // stay go to accel
                            speed_state = 0;
                            // Serial.println("0 -> 0 + > ++ smaller");
                        }
                    }
                ...
                {
                    if (sign(speed_real) == -1 && sign(speed_command) == 1)
                    {
                        Peripheral.cmd_speed_PWM(0);
                        // Peripheral.drv_write(0x00, 0x2122);
                        // Peripheral.cmd_speed_SPI(0);
            ...
            else if (speed_command < speed_real && fabsf(speed_command) > cmd_deadband) 
            {
                if ((sign(speed_command) == sign(speed_real) || sign(speed_real) == 0) && (speed_real - speed_command > deadband))
                {
                    ...
                    {
                        if(speed_real - speed_command > 100.0f)
                        {
                            // Command speed
                            // Peripheral.cmd_speed_PWM(fabsf(speed_command));
                            Peripheral.drv_write(0x00, 0x2122);
                ...
                else
                {
                    if (sign(speed_real) == 1 && sign(speed_command) == -1)
                    {
                        // Command zero and brake the motor
                        Peripheral.cmd_speed_PWM(0); 
                        // Peripheral.drv_write(0x00, 0x2120);
                        // Peripheral.cmd_speed_SPI(0);
                        Peripheral.brake();
            ...
            else if (fabsf(speed_command) < cmd_deadband && fabsf(speed_real) > cmd_deadband) // different condition to avoid chatter between decel and idle at zero
            {
                Peripheral.cmd_speed_PWM(0);
                Peripheral.brake();
                speed_state = 2;
                // Serial.println("0 -> 2 zerospd");
            }
        }
        ...
        {
            // edge count to 2 to improve accuracy at low speeds 
            edge_count.put(2);

            // state transition from accel back to idle, deadband 20rpm
            if (fabsf(speed_real-speed_command) <= deadband || fabsf(speed_real) >= fabsf(speed_command))
            {
                edge_count.put(10);
                // register programming (set SPDMODE to 10) (for clkin mode set 0x2102)
                Peripheral.drv_write(0x00, 0x2102);
                Peripheral.cmd_speed_PWM(fabsf(speed_command));
                // Peripheral.cmd_speed_SPI(fabsf(speed_command));
                speed_state = 0;
                // Serial.println("1 -> 0");
            }
        ...
        else if (speed_state == 2) 
        {
            ...
            { 
                ...
                if (fabsf(speed_command) > 100.0f)
                {
                    if (fabsf(speed_real-speed_command) <= deadband)
                    {
                        ...
                        speed_state = 0;
                        // Serial.println("2 -> 0 non-deadband");
                    }
                }
                else // introduce a 100rpm deadband to prevent state lockup
            ...
            if (sign(speed_command) != sign(speed_real) && sign(speed_command) != 0)
            {
                if (direction == HIGH) // negative to positive
                {
                    ...
                    {
                        speed_state = 3;
                        // Serial.println("Negative to positive 2->3");
                    }
                }
                else // positive to negative
                {
                    ...
                    {
                        speed_state = 4;
                        // Serial.println("Positive to negative 2->4");
                    }
                }
            }
...
void setup()
{
    ...
    xTaskCreate(task_gainScheduling, "Gain Scheduling", 2048, NULL, 2, NULL);
}
